...
def poly_filter_array(array,mask_remove,mask,scan_list,ibegin,polyorder,minfrac=.75):
...
#			array[:,start:start+n] -= tile(np.mean(array[:,start:start+n]*mask[:,start:start+n],axis=1).reshape(nch,1),[1,n])
			for i in range(nch):
				if np.any(mask[i,start:start+n]):
					mean = np.average(array[i,start:start+n],weights=mask[i,start:start+n])
		...
		for s in range(len(scan_list)):
			...
			if n<=polyorder: ## otherwise cannot compute legendre polynomials
                                for i in xrange(nch):
                                        ...
                                        mask_remove[i,start:start+n]=0 #remove this region from actual data as well
...
#					array[i,start:start+n], legendres,rinv,qt)
			...
			for i in xrange(nch):
				...
				if goodhits[i] < minfrac*n: #not enough points
					...
					mask_remove[i,start:start+n]=0 #remove this region from actual data as well
					continue
				bolo,coeff = filter_slice_legendre_qr_mask_precalc(array[i,start:start+n],mask[i,start:start+n],legendres)
